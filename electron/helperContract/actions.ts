import type { HelperContractEntry } from '../helperContract.js';
...
export const helperContract = [
  ...
  {
    frontendApi: 'listSshConfigHosts',
    helperAction: 'list_ssh_config_hosts',
    visibility: 'renderer',
    electronPreloadMethod: 'listSshConfigHosts',
    timeoutClass: 'local-10s',
    dbMutation: 'none',
    pollingOverlapKey: 'none',
    helperEnvelope: 'request:{action:string,payload:object};response:{ok:true,data}|{ok:false,error:{layer,type,message}}',
    fallbackBehavior: 'Read default ~/.ssh/config only; browser fallback returns no candidates plus backend-unavailable warning.',
