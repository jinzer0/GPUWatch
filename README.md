...
수집된 데이터는 backend에서 protocol v1 스냅샷으로 정리되며, 서버 설정, 최신 성공 스냅샷, 최근 24시간 GPU 기록은 로컬 SQLite에 저장됩니다.
GPU 서버의 현재 사용 가능 여부, 프로세스 점유 상태, 최근 24시간의 GPU 지표 흐름을 빠르게 확인하는 데 초점을 둡니다.
장기 감사 로그, raw snapshot history, Prometheus exporter, 원격 collector 설치는 제공하지 않습니다.
...
- Server Detail 차트는 저장된 1시간 GPU 기록을 우선 사용하고, 저장 기록을 불러오는 중이거나 비어 있으면 현재 앱 세션의 live sample로 fallback합니다.
- 저장 GPU 기록은 성공한 poll에서 GPU별로만 추가되며, 실패한 poll은 health와 오류 metadata를 갱신하고 history sample은 만들지 않습니다.
- 기록 보관 기간은 고정 24시간입니다. 장기 audit history나 process timeline은 저장하지 않습니다.
...
- 실패한 새로고침 이후에도 최신 성공 스냅샷을 보존하고 stale 상태와 오류 정보를 표시합니다. 실패한 poll은 저장 history point나 fallback live sample을 추가하지 않습니다.
...
`nvidia-smi` 출력은 드라이버, GPU 모드, MIG 구성에 따라 `N/A` 또는 `-`를 반환할 수 있습니다. MIG 인스턴스 수, PCIe 처리량, process utilization, 런타임 같은 선택 지표도 서버와 드라이버마다 빠지거나 달라질 수 있습니다. GPUWatcher는 이런 값을 0으로 바꾸지 않고 `unknown` 또는 `null`로 처리합니다. 저장 history chart에서도 null metric은 0이 아니라 gap 또는 unknown으로 표시됩니다. nvitop은 참고 기준일 뿐이며, GPUWatcher는 원격 nvitop 실행이나 정확한 NVML plus psutil 동등성을 목표로 하지 않습니다.
