...
- 이슈가 `CLOSED`이면 close 단계는 검증된 no-op으로 처리하며 별도 승인을 요구하지 않는다. 단, 각 worktree/원격 ref/로컬 ref 삭제 직전과 close 직전의 재검증에서 `OPEN`으로 관측되면 exact server `updated_at`를 포함한 approval tuple이 필요하다.
- 이슈가 `OPEN`이면 read-only preflight가 atomically fetched `state`와 exact server `updated_at`를 포함해 출력한 exact approval tuple을 작업지시자가 같은 스레드에서 그대로 식별해 승인해야 한다. 산문형 "close 승인"은 무효다.
- GitHub issue close API에는 atomic compare-and-set이 없다. 따라서 cleanup transaction은 최초 관측한 merged PR state/base/head refs/head repository/head OID와 issue `state`/exact server `updated_at`를 immutable tuple로 고정한다. 각 worktree/원격 ref/로컬 ref 삭제 직전과 close 직전에 live PR/issue state/updated_at tuple을 다시 읽어 frozen tuple과 비교하고, 각 경계에서 관측된 `OPEN` 이슈에 대해 frozen tuple의 exact approval을 검증하며, close 후 `CLOSED` 상태를 다시 확인한다.
...
1. 대상과 실행 위치 사전 확인 (read-only)
   ...
   readonly FROZEN_PR_STATE FROZEN_PR_BASE_REF FROZEN_PR_HEAD_REF FROZEN_PR_HEAD_REPOSITORY FROZEN_MERGED_HEAD_OID
     ...
     ISSUE_TUPLE="$(GH_HOST="$CANONICAL_HOST" gh api --hostname "$CANONICAL_HOST" --method GET "repos/$CANONICAL_REPOSITORY/issues/$ISSUE_NUMBER" --jq '[.number, .state, .updated_at] | @tsv')"
     case "$ISSUE_TUPLE" in *$'\n'*) fail 'Issue state query returned multiple lines' ;; esac
     IFS=$'\t' read -r RETURNED_ISSUE_NUMBER ISSUE_STATE_RAW ISSUE_UPDATED_AT ISSUE_EXTRA_FIELD <<< "$ISSUE_TUPLE"
     test -z "${ISSUE_EXTRA_FIELD:-}" || fail 'Issue state query returned extra fields'
     test "$RETURNED_ISSUE_NUMBER" = "$ISSUE_NUMBER" || fail 'Issue response number mismatch'
     case "$ISSUE_STATE_RAW" in
       open) ISSUE_STATE=OPEN ;;
       closed) ISSUE_STATE=CLOSED ;;
       *) fail 'Issue state must be OPEN or CLOSED' ;;
     esac
    validate_scalar ISSUE_UPDATED_AT "$ISSUE_UPDATED_AT"
   ...
   printf 'expected_pr_tuple state=%s base_ref=%s head_ref=%s head_repository=%s merged_head_oid=%s\n' \
    ...
    printf 'approval_tuple host=%s repository=%s repository_id=%s pr_number=%s issue_number=%s issue_state=%s issue_updated_at=%s merged_head_oid=%s base_ref=%s head_ref=%s action=%s\n' \
      "$CANONICAL_HOST" "$CANONICAL_REPOSITORY" "$CANONICAL_REPOSITORY_ID" "$PR_NUMBER" "$ISSUE_NUMBER" "$ISSUE_STATE" "$ISSUE_UPDATED_AT" "$FROZEN_MERGED_HEAD_OID" "$FROZEN_PR_BASE_REF" "$FROZEN_PR_HEAD_REF" "$ACTION"
    printf 'issue_state=%s\nissue_updated_at=%s\n' "$ISSUE_STATE" "$ISSUE_UPDATED_AT"
   ...
   - 작업지시자가 지정한 PR/이슈 번호, `MERGED`, `devel` <- `publish/taskN`, canonical repository, canonical origin, 기본/분리 worktree 경로를 확인한다.
    - cleanup transaction에는 이슈 상태와 무관하게 `expected_pr_tuple`의 값과 atomically fetched `ISSUE_STATE`, `ISSUE_UPDATED_AT`를 `PREFLIGHT_PR_STATE`, `PREFLIGHT_PR_BASE_REF`, `PREFLIGHT_PR_HEAD_REF`, `PREFLIGHT_PR_HEAD_REPOSITORY`, `PREFLIGHT_MERGED_HEAD_OID`, `PREFLIGHT_ISSUE_STATE`, `PREFLIGHT_ISSUE_UPDATED_AT`로 그대로 전달한다.
    - `OPEN` 이슈라면 같은 스레드 승인 문장이 `approval_tuple`의 host, repository, repository ID, PR 번호, 이슈 번호, issue state, exact server `updated_at`, merged head OID, base/head refs, action `close-issue:completed`를 모두 그대로 식별해야 한다.

2. 승인받은 cleanup transaction 실행
    - 이슈가 `CLOSED` 또는 `OPEN`인지와 무관하게 read-only preflight가 출력한 frozen PR tuple과 `PREFLIGHT_ISSUE_STATE`, `PREFLIGHT_ISSUE_UPDATED_AT` 환경 변수를 그대로 전달한다.
    - 이슈가 `OPEN`으로 관측된 경우 승인받은 tuple 값을 `APPROVED_HOST`, `APPROVED_REPOSITORY`, `APPROVED_REPOSITORY_ID`, `APPROVED_PR_NUMBER`, `APPROVED_ISSUE_NUMBER`, `APPROVED_ISSUE_STATE`, `APPROVED_ISSUE_UPDATED_AT`, `APPROVED_MERGED_HEAD_OID`, `APPROVED_BASE_REF`, `APPROVED_HEAD_REF`, `APPROVED_ACTION` 환경 변수로 그대로 전달한다.
   ...
   validate_approved_tuple() {
     ...
     validate_decimal APPROVED_REPOSITORY_ID "${APPROVED_REPOSITORY_ID:-}"
      ...
      validate_scalar APPROVED_ISSUE_STATE "${APPROVED_ISSUE_STATE:-}"
      validate_scalar APPROVED_ISSUE_UPDATED_AT "${APPROVED_ISSUE_UPDATED_AT:-}"
     ...
     test "$APPROVED_REPOSITORY_ID" = "$CANONICAL_REPOSITORY_ID" || fail 'approved repository ID mismatch'
      ...
      test "$APPROVED_ISSUE_STATE" = "$FROZEN_ISSUE_STATE" || fail 'approved issue state mismatch'
      test "$APPROVED_ISSUE_UPDATED_AT" = "$FROZEN_ISSUE_UPDATED_AT" || fail 'approved issue updated_at mismatch'
   ...
   observe_pr_issue_tuple() {
     ...
     validate_oid LIVE_PR_HEAD_OID "$LIVE_PR_HEAD_OID"
      ...
      case "$ISSUE_TUPLE" in
        *$'\n'*)
          fail 'Issue state query returned multiple lines'
          return 1
          ;;
      esac
      IFS=$'\t' read -r RETURNED_ISSUE_NUMBER ISSUE_STATE_RAW LIVE_ISSUE_UPDATED_AT ISSUE_EXTRA_FIELD <<< "$ISSUE_TUPLE"
      if test -n "${ISSUE_EXTRA_FIELD:-}"; then
        fail 'Issue state query returned extra fields'
        return 1
      fi
      if ! test "$RETURNED_ISSUE_NUMBER" = "$ISSUE_NUMBER"; then
        fail 'Issue response number mismatch'
        return 1
      fi
      case "$ISSUE_STATE_RAW" in
       open) ISSUE_STATE=OPEN ;;
      ...
      validate_scalar LIVE_ISSUE_UPDATED_AT "$LIVE_ISSUE_UPDATED_AT" || return 1
   }
   revalidate_pr_issue_tuple() {
     observe_pr_issue_tuple || return 1
     ...
     test "$LIVE_PR_HEAD_REF" = "$FROZEN_PR_HEAD_REF" || fail 'PR head ref changed after preflight'
      ...
      test "$ISSUE_STATE" = "$FROZEN_ISSUE_STATE" || fail 'Issue state changed after preflight'
      test "$LIVE_ISSUE_UPDATED_AT" = "$FROZEN_ISSUE_UPDATED_AT" || fail 'Issue updated_at changed after preflight'
   }
   ...
   EXPECTED_TASK_BRANCH="local/task${ISSUE_NUMBER}"
    ...
    read_local_task_ref() {
      ...
      LOCAL_TASK_OID=""
      if git symbolic-ref --quiet "$LOCAL_TASK_REF" >/dev/null; then
        fail 'local task ref must not be symbolic'
        return 1
      fi
   ...
   validate_scalar PREFLIGHT_PR_HEAD_REPOSITORY "${PREFLIGHT_PR_HEAD_REPOSITORY:-}"
    ...
    validate_scalar PREFLIGHT_ISSUE_STATE "${PREFLIGHT_ISSUE_STATE:-}"
    validate_scalar PREFLIGHT_ISSUE_UPDATED_AT "${PREFLIGHT_ISSUE_UPDATED_AT:-}"
   ...
   test "$PREFLIGHT_PR_BASE_REF" = "$EXPECTED_BASE_REF" || fail 'preflight PR base ref mismatch'
    ...
    case "$PREFLIGHT_ISSUE_STATE" in OPEN|CLOSED) ;; *) fail 'preflight issue state mismatch' ;; esac
   ...
   FROZEN_PR_HEAD_REPOSITORY="$PREFLIGHT_PR_HEAD_REPOSITORY"
    ...
    FROZEN_ISSUE_STATE="$PREFLIGHT_ISSUE_STATE"
    FROZEN_ISSUE_UPDATED_AT="$PREFLIGHT_ISSUE_UPDATED_AT"
    readonly FROZEN_PR_STATE FROZEN_PR_BASE_REF FROZEN_PR_HEAD_REF FROZEN_PR_HEAD_REPOSITORY FROZEN_MERGED_HEAD_OID FROZEN_ISSUE_STATE FROZEN_ISSUE_UPDATED_AT
   revalidate_pr_issue_tuple
   require_approved_tuple_if_issue_open
   ...
   if test -n "$LOCAL_TASK_OID"; then
     revalidate_pr_issue_tuple
     require_approved_tuple_if_issue_open
       git update-ref --no-deref -d "refs/heads/${EXPECTED_TASK_BRANCH}" "$LOCAL_TASK_OID"
       ! git show-ref --verify --quiet "refs/heads/${EXPECTED_TASK_BRANCH}" || fail 'local task ref remains after deletion'
       ! git symbolic-ref --quiet "refs/heads/${EXPECTED_TASK_BRANCH}" >/dev/null || fail 'local task symbolic ref remains after deletion'
   fi
   ...
   - read-only preflight의 merged PR tuple을 frozen tuple로 고정 -> local task/remote publish ref 검증 -> frozen PR/이슈 tuple과 현재 `OPEN` 이슈의 exact approval을 각 경계에서 재검증 -> worktree 제거 -> 재검증 -> 원격 publish branch 삭제 -> 재검증 -> 로컬 task branch 삭제 -> 재검증 -> 이슈 close 또는 `CLOSED` no-op 검증 순서를 유지한다.
    - 처음 `CLOSED`였던 이슈가 어느 destructive boundary 직전 재검증에서 `OPEN`이면 `APPROVED_*` tuple 누락 또는 불일치로 다음 삭제나 close 전에 중단한다. frozen PR tuple의 state/base/head refs/head repository/head OID 또는 frozen issue `state`/`updated_at` 중 하나라도 달라지면 다음 destructive action 전에 중단한다. artifact 정리 후 close 직전 검증에 실패하면 close 없이 중단한다. 이 경우 standalone close를 실행하지 말고 read-only preflight와 cleanup transaction 전체를 다시 실행한다. preflight부터 `CLOSED`였고 exact `updated_at`가 유지된 경우만 close 단계는 검증된 no-op이다.
- worktree 제거, 원격 publish branch 삭제, 로컬 task branch 삭제 중 하나라도 실패하면 `set -e`로 중단되어 이슈 close에 도달하지 않는다. 로컬 task ref는 symbolic ref를 거부하고 exact OID CAS `git update-ref --no-deref -d` 뒤 exact ref absence를 확인한다.
...
- 각 worktree/원격 ref/로컬 ref 삭제 직전과 close 직전에 live PR tuple이 최초 frozen merged PR state/base/head refs/head repository/head OID와 일치하고, issue `updated_at`가 preflight/approval exact scalar와 일치하며, 그 경계에서 `OPEN`으로 관측된 이슈는 같은 스레드 승인 문장이 host, repository, repository ID, PR 번호, 이슈 번호, issue state/updated_at, frozen merged head OID, base/head refs, action `close-issue:completed` exact tuple을 모두 식별했음을 확인
