...
- 이슈가 이미 `CLOSED`이면 close 단계는 검증된 no-op으로 처리하며 별도 승인을 요구하지 않는다.
- 이슈가 `OPEN`이면 read-only preflight가 출력한 exact approval tuple을 작업지시자가 같은 스레드에서 그대로 식별해 승인해야 한다. 산문형 "close 승인"은 무효다.
- GitHub issue close API에는 atomic compare-and-set이 없다. 따라서 cleanup 실행은 mutation 전과 close 직전에 동일 PR/이슈 tuple을 재조회해 검증하고, close 후 `CLOSED` 상태를 다시 확인한다.
...
1. 대상과 실행 위치 사전 확인 (read-only)
   ...
   test "$PR_STATE" = "MERGED" || fail 'PR must be MERGED'
   test "$PR_BASE_REF" = "$EXPECTED_BASE_REF" || fail 'PR base ref mismatch'
   test "$PR_HEAD_REF" = "$EXPECTED_HEAD_REF" || fail 'PR head ref mismatch'
   test "$PR_HEAD_REPOSITORY" = "$CANONICAL_REPOSITORY" || fail 'PR head repository mismatch'
   validate_oid PR_HEAD_OID "$PR_HEAD_OID"
   ...
   printf 'approval_tuple host=%s repository=%s repository_id=%s pr_number=%s issue_number=%s merged_head_oid=%s base_ref=%s head_ref=%s action=%s\n' \
     "$CANONICAL_HOST" "$CANONICAL_REPOSITORY" "$CANONICAL_REPOSITORY_ID" "$PR_NUMBER" "$ISSUE_NUMBER" "$PR_HEAD_OID" "$EXPECTED_BASE_REF" "$EXPECTED_HEAD_REF" "$ACTION"
...
2. 승인받은 cleanup transaction 실행
   - 이슈가 `OPEN`이었던 경우 승인받은 tuple 값을 `APPROVED_HOST`, `APPROVED_REPOSITORY`, `APPROVED_REPOSITORY_ID`, `APPROVED_PR_NUMBER`, `APPROVED_ISSUE_NUMBER`, `APPROVED_MERGED_HEAD_OID`, `APPROVED_BASE_REF`, `APPROVED_HEAD_REF`, `APPROVED_ACTION` 환경 변수로 그대로 전달한다.
   - 이슈가 이미 `CLOSED`인 경우 `APPROVED_*` 입력 없이 실행할 수 있다.
   ...
   validate_approved_tuple() {
     ...
     test "$APPROVED_MERGED_HEAD_OID" = "$PR_HEAD_OID" || fail 'approved merged head OID mismatch'
     test "$APPROVED_BASE_REF" = "$EXPECTED_BASE_REF" || fail 'approved base ref mismatch'
     test "$APPROVED_HEAD_REF" = "$EXPECTED_HEAD_REF" || fail 'approved head ref mismatch'
   ...
   revalidate_pr_issue_tuple() {
     ...
     IFS=$'\t' read -r PR_STATE PR_BASE_REF PR_HEAD_REF PR_HEAD_REPOSITORY PR_HEAD_OID EXTRA_FIELD <<< "$PR_TUPLE"
     test -z "${EXTRA_FIELD:-}" || fail 'PR identity query returned extra fields'
     test "$PR_STATE" = "MERGED" || fail 'PR must stay MERGED'
     test "$PR_BASE_REF" = "$EXPECTED_BASE_REF" || fail 'PR base ref mismatch'
     test "$PR_HEAD_REF" = "$EXPECTED_HEAD_REF" || fail 'PR head ref mismatch'
     test "$PR_HEAD_REPOSITORY" = "$CANONICAL_REPOSITORY" || fail 'PR head repository mismatch'
     validate_oid PR_HEAD_OID "$PR_HEAD_OID"
   ...
   revalidate_pr_issue_tuple
   REQUIRE_CLOSE_APPROVAL=0
   if test "$ISSUE_STATE" = "OPEN"; then
     validate_approved_tuple
     REQUIRE_CLOSE_APPROVAL=1
   else
     printf 'issue_already_closed=verified-no-op\n'
   fi
   ...
   git merge-base --is-ancestor "$PR_HEAD_OID" origin/devel
   ...
   revalidate_pr_issue_tuple
   if test "$REQUIRE_CLOSE_APPROVAL" = "1"; then
     validate_approved_tuple
   fi
   if test "$ISSUE_STATE" = "OPEN"; then
   ...
   - worktree 제거 -> 원격 publish branch 삭제 -> 로컬 task branch 삭제 -> 즉시 PR/이슈 tuple 재검증 -> 이슈 close 또는 CLOSED no-op 검증 순서를 유지한다.
...
- `OPEN` 이슈였던 경우 같은 스레드 승인 문장이 host, repository, repository ID, PR 번호, 이슈 번호, merged head OID, base/head refs, action `close-issue:completed` exact tuple을 모두 식별했음을 확인
- cleanup 시작 전과 close 직전에 PR/이슈 tuple이 승인된 exact tuple 및 canonical repository와 일치
...
- cleanup 도중 이슈가 이미 `CLOSED`가 되었으면 close 명령을 생략하고 `CLOSED` 상태를 다시 확인
