...
- 처리 중인 최신 round만 위 이름을 사용한다. 처리 완료 문서는 basename과 상대 링크를 유지한 채 `mydocs/pr/archives/pr_{번호}_round{양의 정수}/`로 이동하며, 기존 archive와 충돌하면 다음 빈 round 번호를 사용하고 덮어쓰지 않는다.
