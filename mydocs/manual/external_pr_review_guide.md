...
- 이 문서는 정책 요약이다. 실행 가능한 canonical source는 `mydocs/skills/external-pr-review/SKILL.md`이며, archive와 rollback 세부 검증은 Skill의 현재 Bash block과 테스트 결과를 기준으로 한다.
...
8. temporary refs가 absent인지 확인하고 snapshot root를 제거한다. snapshot root는 Step 1 output의 device:inode와 현재 root identity를 맞춘 뒤 unique sibling으로 claim하고, claimed root와 exact private member의 ownership, link count, mode가 그대로일 때만 삭제한다. root identity mismatch, replacement, unknown member는 원본 또는 claimed root를 보존하고 fail-closed로 실패한다. 두 결과가 확인된 뒤에만 `pr_{번호}_report.md`를 작성한다.
...
11. 작업지시자가 같은 스레드에서 approval tuple 전체를 승인한 뒤에만 archive commit을 수행한다. Round 1 untracked source는 archive destination addition만 stage하고 tracked source는 source와 destination을 stage해 exact rename으로 처리한다. mixed state는 per-file tuple state와 정확히 일치해야 한다. move 전에는 archive root ownership, source filesystem identity, source/destination index entry, destination absence를 capture하고 다시 replay한다. review, report, optional implementation의 세 source는 post-stage destination entry, source entry, blob, SHA-256, byte count를 각각 재검증한다. filesystem SHA-256과 byte 읽기는 `$REPO_ROOT` 기준 absolute path로 고정하고, blob 읽기는 `git -C "$REPO_ROOT"`에 repository-relative destination을 넘겨 nested CWD에서도 같은 결과를 요구한다. commit 전후 branch/parent/subject와 exact staged/committed tree, name-status, destination `100644` mode/blob을 검증하고 exported `GIT_CONFIG_COUNT=1`, `GIT_CONFIG_KEY_0=core.hooksPath`, `GIT_CONFIG_VALUE_0=/dev/null` tuple로 hooks를 비활성화한 repository-wide index와 worktree가 clean인지 확인한다.
12. archive 실패 rollback은 소유권을 증명한 exact path만 다룬다. ref가 created OID의 direct `exact-new` 상태일 때만 같은 FIFO-backed no-deref exact-old transaction으로 parent에 되돌리고, `exact-old`는 ref advance가 없었던 상태로만 검증한다. competing direct ref, symref, missing, unreadable ref는 보존하며 rollback mutation을 하지 않는다. file rollback도 moved destination의 device:inode, owner, link count, SHA-256, byte count, 원래 mode, source 부재, pre-CAS source/destination index entry, post-stage ownership replay가 모두 맞을 때만 source로 되돌린다. 단순 destination-to-source 복구를 안전하다고 쓰지 않는다. HUP/INT/TERM 같은 outer signal은 rollback trap으로 들어가며, 검증이 하나라도 깨지면 repository 상태를 보존하고 실패한다.
